...
export class Renderer {
    ...
    drawGrid(walkableCells: Set<string>) {
        ...
        for (let screenY = startY; screenY < this.canvas.height + patternSize; screenY += patternSize) {
            for (let screenX = startX; screenX < this.canvas.width + patternSize; screenX += patternSize) {
                ...
                this.ctx.drawImage(
                    img,
                    tx * tileSizeX, ty * tileSizeY, tileSizeX, tileSizeY,
                    screenX, screenY, patternSize, patternSize
                );
        ...
        this.ctx.strokeRect(offsetX, offsetY, this.width * this.cellSize, this.height * this.cellSize);
    }
    ...
    drawObstacles(obstacles: { pos: Point, type: 'wood' | 'iron' }[]) {
        ...
        obstacles.forEach((obs) => {
            const img = AssetLoader.getImage(obs.type === 'iron' ? 'crate_metal' : 'crate_wood');
    ...
    drawPortals(portals: { pos: Point, color: string }[]) {
        ...
        portals.forEach((p) => {
            ...
            if (baseImg && glowImg) {
                ...
                this.ctx.beginPath();
                this.ctx.arc(drawX + size / 2, drawY + size / 2, size * 0.4, 0, Math.PI * 2);
                this.ctx.stroke();
                this.ctx.shadowBlur = 0;
            }
        });
    ...
    private getColoredPortal(glowImg: HTMLImageElement, color: string): HTMLCanvasElement {
